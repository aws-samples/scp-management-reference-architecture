...
├── control_policy_module         # <-- Terraform module for creating SCPs and attaching them to targets
├── control_policies_ou_structure # <-- a directory with sub-directories specific to the OUs to which SCPs are directly attached
...
├── (cp_define_attach_auto.tf)  # <-- [NOT PRESENT IN CODE, WILL BE AUTO-GENERATED IN PIPELINE] main terraform file that is dynamically generated by the resolve_control_policy_data.py script.
├── README.md                    # <-- This file
└── control-policy-pipeline.yaml # <-- CloudFormation template used to create the SCP CodePipeline - can be removed if you have an existing pipeline solution
```
...
1. **_`generate_policies_ou_structure_and_imports.py`_** - This script is used to set up the initial OU structure and Terraform import files. It helps accelerate the use of this solution. It should be run manually when first implementing the solution, as well as any time there are OU structure changes (eg. new accounts added, OU renames, OU restructures).
2. **_`resolve_control_policy_data.py`_** - This script runs during the pipeline execution. It should not be manually run. This script will dynamically generate Terraform code based on the contents of the `service_control_policies` OU structure folder.
...
1. Re-run the `generate_policies_ou_structure_and_imports.py --skip-import-creation --skip-customer-scp-refresh` script and modify as necessary to align SCPs to the new structure.
...
2. Using CLI credentials from the SCP account, run the script `generate_scp_ou_structure_and_import.py`
   1. This script will generate the following content:
      1. A `control_policies_ou_structure` folder that contains a folder structure mirroring the OU structure of the Organization. Within this folder, there are two subfolders: `ROOT` and `SHARED`. Within these folders are JSON files with Control Policies.
...
3. Configure a pipeline to run this solution
   1. [Option 1] If you need to deploy a pipeline specifically for this solution and don't have a standard pipeline solution for your organization: Deploy the CloudFormation template `scp-pipeline.yaml` to create a pipeline that uses Terraform to manage CPs.
      ...
      ```
   2. [Option 2] If you DO have a standard pipeline solution, you will need to ensure that you run the script `resolve_control_policy_data.py` before any `terraform plan/apply` actions. This ensures that the JSON data and folder structure gets translated into attached SCPs.
...
6. Update your repository to include the initial files (including those generated by `generate_scp_ou_structure_and_import.py`) using `git`. Make sure you include the `control_policies_ou_structure` directory, `control_policy_module` directory, import manifests, `backend.tf` manifest, and `resolve_control_policy_data.py` file at a minimum.
