...
2. Delete the existing `service_control_policies` folder and recreate it via script. Using CLI credentials from SCP account, run the script `generate_scp_ou_structure_and_import.py`
...
4. [Optional] If you do not already have a Terraform state bucket and Terraform DynamoDB lock table for backending Terraform, use the `bootstrap` folder to deploy those resources. You will need permissions to create an S3 bucket and a DynamoDB table for this.
