...
resource "aws_s3_bucket" "tf_remote_state_bucket" {
  ...
  bucket = "iam-identitycenter-${data.aws_caller_identity.current.account_id}-tf-state"
  tags = {
    terraform = "True"
  }
...
resource "aws_s3_bucket_policy" "this_account" {
  bucket = aws_s3_bucket.tf_remote_state_bucket.id
  policy = data.aws_iam_policy_document.this_account.json
}

data "aws_iam_policy_document" "this_account" {
  statement {
    principals {
      type        = "AWS"
    ...
    resources = [
      aws_s3_bucket.tf_remote_state_bucket.arn,
      "${aws_s3_bucket.tf_remote_state_bucket.arn}/*",
    ]
  }
}